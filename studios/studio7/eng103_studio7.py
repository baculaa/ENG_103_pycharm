...
class Node:
	def __init__(self, Xcoord = 0, Ycoord=0, Frame=0, Time=0, Value=0, Color = [1,0,0], Angle = 0):
		self.Xcoord = Xcoord
		self.Ycoord = Ycoord
		self.Frame  = Frame
		self.Time   = Time
...
class Studio_Functions:	
	def Find_Frame_With_MaxVal(self, GrandList, X_index, Y_index):
		...
		for Frame in range(Num_Frames):
			...
			#------------------------------
			###### EDIT HERE (BEGIN) ###### (Change one line)	
			
			Current_Val = 0 # CHANGE THIS LINE!! # Set Current_Val to the Value of the Node at our X_index and Y_index, and at this current Frame. 
	...
	def Find_Value_vs_Time(self, GrandList, X_Index, Y_Index):
		...
		for Frame in range(Num_Frames):
			...
			# To do so, we'll have to properly index our GrandList	
			#------------------------------
			###### EDIT HERE (BEGIN) ###### (Change two lines)	
			
			Current_Time = 0 # CHANGE THIS LINE! # Set Current_Time to Time  property of the node at this X_index,Y_index,and Frame.
			Current_Val  = 0 # CHANGE THIS LINE! # Set Current_Val  to Value property of the node at this X_index,Y_index,and Frame.
		 
			# If you index the GrandList as GrandList[Index][Index][Index], you'll get a Node, which has properties like Xcoord, Value, Color, Time, etc.			 
			###### EDIT HERE (END) ######
			#------------------------------
		...
		for Frame in range(Num_Frames):
			...
			for X in range(Nx):
				for Y in range(Ny):
					#------------------------------
					###### EDIT HERE (BEGIN) ###### (Change one line)	
					Total  += 0 # CHANGE THIS LINE! # ADD this node's value to the running Total.
					# The Total should INCREASE by the value of the node at this X, Y,and Frame.
					###### EDIT HERE (END) ######
					#------------------------------
			Totals.append(Total)
		...
		for Frame in range(Num_Frames):
			for X in range(Nx):
				for Y in range(Ny):
					...
					Brightness = 0 # !! CHANGE THIS LINE !! # Set Brightness equal to the average of RGB values defining this node's color.		
					# For example, A color of [0.25, 0.50, 0.75] should have a brightness of 0.50	
					
					if Brightness >= Max_Brightness:
						Max_Brightness = Brightness
						Brightest = Node() # !! CHANGE THIS LINE !! # Set Brightest equal to the node at this X, Y and Frame
						# It should look like this: Brightest = GrandList[Index][Index][Index]				
...
if __name__ == '__main__':
	...
	Control_Func = H.Control_Function_2 # Function to control value of pixels
	...
	plt.title('Part 4: Find The Brightest Star. Brightness = %.2f' %(sum(Brightest.Color)/3))
